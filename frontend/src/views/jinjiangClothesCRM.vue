<template>
  <div class="container">
    ...
    <MenuDrawer v-model:visible="menuOpen" current="jinjiang" />
  </div>
</template>
...
import { jinjiangClothesData } from '../data/jinjiangClothes.js'
import MenuDrawer from '../components/MenuDrawer.vue'

const STORAGE_KEY = 'jinjiang_clothes_companies'
const STATUS_KEY = 'jinjiang_clothes_status'
const NOTES_KEY = 'jinjiang_clothes_notes'
...
onMounted(() => {
  ...
  try {
    companies.value = (jinjiangClothesData && jinjiangClothesData.length)
      ? jinjiangClothesData.map((r, i) => ({
        id: String(i + 1),
        name: r.company || r.name || '',
        boss: r.boss || r.owner || r.contact || '',
        address: r.address || '',
        phone: [r.usefulPhone, r.morePhone, r.phone].filter(v => !!v && String(v).trim() && String(v).trim() !== '-').join(';'),
        raw: r,
      }))
      : []
...
function splitPhones(v) {
  return String(v || '').split(/[；;]/).map(s => s.trim()).filter(s => !!s && s !== '-' && s !== '—')
}
