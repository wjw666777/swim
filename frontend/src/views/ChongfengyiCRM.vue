      ...
      <div class="detail">
        <div>
          <a-descriptions :column="1" bordered size="small">
            ...
            <a-descriptions-item label="电话">{{ current?.raw?.usefulPhone || current?.raw?.mobile || current?.phone }}</a-descriptions-item>
            <a-descriptions-item label="更多电话">{{ current?.raw?.morePhone || current?.raw?.phones || '-' }}</a-descriptions-item>
...
import { API_BASE, fetchChongfengyiCompanies, deleteChongfengyiCompany, updateChongfengyiCompany, fetchChongfengyiVisitStatus, updateChongfengyiVisitStatus, fetchChongfengyiVisitLogs, addChongfengyiVisitLog } from '../api/client.js'
...
async function saveDetailRecord() {
  ...
  if (!id) return
  console.log('[Chongfengyi] API_BASE =', API_BASE)
  try {
    // 先更新本地状态以立即反馈到列表标签
    visitStatus.value[id] = detailStatus.value
    localStorage.setItem(VISIT_STATUS_KEY, JSON.stringify(visitStatus.value))
    // 再尝试提交到后端
    await updateChongfengyiVisitStatus(id, detailStatus.value)
  } catch (e) {
    // 后端不可用时保留本地更新，并给予提示
    console.warn('updateChongfengyiVisitStatus failed:', e)
    Message.warning('拜访状态后端未更新（已本地保存）')
  }
  // 保存回访记录（接口）
  try {
    const note = (detailNotes.value || '').trim()
    if (note) {
      await addChongfengyiVisitLog(id, { date: new Date().toISOString(), note })
      // 验证后端写入
      try {
        const logs = await fetchChongfengyiVisitLogs(id)
        if (Array.isArray(logs) && logs.length > 0) {
          Message.success(`回访记录已保存到后端（共${logs.length}条）`)
        } else {
          Message.warning('后端未返回记录，请检查API地址或网络')
        }
      } catch (verifyErr) {
        console.warn('verify logs failed:', verifyErr)
        Message.warning('回访记录验证失败（可能是网络/后端问题）')
      }
    }
  } catch (e) {
    console.warn('addChongfengyiVisitLog failed:', e)
    Message.warning(`回访记录未保存到后端（API: ${API_BASE || '未配置'}）`)
  }
  Message.success('已保存')
...
async function confirmEdit() {
  try {
    await updateChongfengyiCompany(editForm.value.id, { name: editForm.value.name, boss: editForm.value.boss, phone: editForm.value.phone })
...
async function remove(record) {
  try {
    await deleteChongfengyiCompany(record.id)
    companies.value = companies.value.filter(c => c.id !== record.id)
    Message.success('已删除')
  } catch (e) {
    Message.warning('离线模式：仅本地删除')
    companies.value = companies.value.filter(c => c.id !== record.id)
  }
}
...
async function loadData() {
  loading.value = true
  try {
    const apiData = await fetchChongfengyiCompanies()
    if (Array.isArray(apiData) && apiData.length) {
      // 兼容新数据(jacketsData)与旧数据字段
      companies.value = apiData.map((r, i) => ({
        id: r.id || String(i + 1),
      ...
      // 合并拜访状态（后端优先，失败回退本地）
      try { visitStatus.value = await fetchChongfengyiVisitStatus() }
      catch { visitStatus.value = JSON.parse(localStorage.getItem(VISIT_STATUS_KEY) || '{}') }
      localStorage.setItem('chongfengyi_companies', JSON.stringify(companies.value))
    } else {
      const saved = localStorage.getItem('chongfengyi_companies')
      if (saved) {
        companies.value = JSON.parse(saved)
        Message.info('已从本地缓存加载数据')
      } else {
        companies.value = (jacketsData && jacketsData.length)
          ? jacketsData.map((r, i) => ({ id: r.id || String(i + 1), name: r.company || r.name || '', boss: r.boss || '', phone: r.usefulPhone || r.morePhone || r.phone || '', raw: r }))
          : []
        Message.info('已使用本地JS数据（jacketsData）')
      }
    }
  } catch (e) {
    const saved = localStorage.getItem('chongfengyi_companies')
    if (saved) {
      companies.value = JSON.parse(saved)
      Message.warning('离线模式：使用本地缓存')
    } else if (jacketsData && jacketsData.length) {
      companies.value = jacketsData.map((r, i) => ({ id: r.id || String(i + 1), name: r.company || r.name || '', boss: r.boss || '', phone: r.usefulPhone || r.morePhone || r.phone || '', raw: r }))
      Message.warning('离线模式：使用本地JS数据（jacketsData）')
    } else {
      companies.value = []
      Message.warning('离线模式：无可用数据（已移除旧 chongfengyiData/Excel 回退）')
    }
  } finally {
    loading.value = false
  }
