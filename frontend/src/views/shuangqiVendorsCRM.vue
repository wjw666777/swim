  ...
  <div class="container">
    ...
    <div class="search-bar">
      <div class="search-container">
        <a-input-search allow-clear placeholder="搜索商家、老板或电话..." v-model="query" @search="onSearch"
          class="search-input" />
      </div>
    </div>
    ...
    <MenuDrawer v-model:visible="menuOpen" current="shuangqi" />
...
import { shuangqiVendorsData } from '../data/shuangqiVendors.js'

const STORAGE_KEY = 'shuangqi_vendors_companies'
const STATUS_KEY = 'shuangqi_vendors_status'
const NOTES_KEY = 'shuangqi_vendors_notes'
...
onMounted(() => {
  ...
  try {
    companies.value = (shuangqiVendorsData && shuangqiVendorsData.length)
      ? shuangqiVendorsData.map((r, i) => ({ id: String(i + 1), name: r.company || '', boss: r.boss || '', address: r.address || '', phone: [r.usefulPhone, r.morePhone].filter(v => !!v && String(v).trim() && String(v).trim() !== '-').join(';'), raw: r }))
      : []
