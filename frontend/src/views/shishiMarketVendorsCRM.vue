  ...
  <div class="container">
    ...
    <div class="search-bar">
      <div class="search-container">
        <a-input-search disabled allow-clear placeholder="搜索商家、老板或电话..." v-model="query" @search="onSearch"
          class="search-input" />
      </div>
    </div>
    ...
    <div class="cards-container" v-else>
      <a-space direction="vertical" :size="12" style="width: 100%">
        <a-card v-for="(company, i) in filteredCompanies" :key="company.id" hoverable @click="openDetail(company)">
          ...
          <template #extra><a-tag :color="statusTagColor(visitStatus[company.id] || 1)">{{
            statusText(visitStatus[company.id] || 1) }}</a-tag></template>
          <div class="card-meta">
          ...
          <div class="card-actions"><a-button type="primary" size="small"
              @click.stop="openDetail(company)">详情</a-button>
          </div>
        </a-card>
      </a-space>
    </div>

    <MenuDrawer v-model:visible="menuOpen" current="shishiMarket" />

    <a-drawer v-model:visible="showDetail" placement="bottom" :height="isMobile ? '85vh' : '70vh'" :footer="false"
      :title="currentCompany?.name || '详情'">
      <div class="detail-grid">
        ...
        <div class="detail-item">
          ...
          <div class="detail-value">{{ (currentCompany?.raw?.people || '-') + ' / ' + (currentCompany?.raw?.scale ||
            '-') }}
          </div>
        </div>
        <div class="detail-item">
      ...
      <a-space direction="vertical" :size="12" style="width: 100%">
        ...
        <div style="text-align: right; margin-top: 8px"><a-button @click="closeDetail">取消</a-button><a-button
            type="primary" style="margin-left: 8px" @click="saveRecord">保存</a-button></div>
      </a-space>
    </a-drawer>
  </div>
</template>
...
import { shishiMarketVendorsData } from '../data/shishiMarketVendors.js'

const STORAGE_KEY = 'shishi_market_vendors_companies'
const STATUS_KEY = 'shishi_market_vendors_status'
const NOTES_KEY = 'shishi_market_vendors_notes'
...
onMounted(() => {
  ...
  try {
    companies.value = (shishiMarketVendorsData && shishiMarketVendorsData.length)
      ? shishiMarketVendorsData.map((r, i) => ({ id: String(i + 1), name: r.company || '', boss: r.boss || '', address: r.address || '', phone: [r.usefulPhone, r.morePhone].filter(v => !!v && String(v).trim() && String(v).trim() !== '-').join(';'), raw: r }))
      : []
