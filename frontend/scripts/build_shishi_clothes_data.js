#!/usr/bin/env node
/**
 * Convert 石狮市服装公司.xlsx to a JS module exporting `shishiClothesData`.
 * Usage:
 *   node scripts/build_shishi_clothes_data.js --xlsx ./src/xlsx/石狮市服装公司.xlsx --out ./src/data/shishiClothes.js [--sheet <nameOrIndex>]
 */
import fs from 'fs'
...
function parseArgs() {
  ...
  const out = { xlsx: '', out: path.join(process.cwd(), 'src/data/shishiClothes.js'), sheet: '' }
...
function isBadHeaders(keys) {
  ...
  const hasMeaningful = keys.some(k => /公司|企业|名称|地址|电话|法人|负责人|省|市|区|注册|参保|规模|简介|经营范围/.test(String(k)))
...
function smartParseSheet(sh) {
  ...
  if (matrix.length >= 2) {
    ...
    headerKeys = hdr
    if (!isBadHeaders(headerKeys)) return { rows: dataRows, headerKeys, rangeUsed: 0 }
  }
  return { rows: XLSX.utils.sheet_to_json(sh, { defval: '' }), headerKeys, rangeUsed: 0 }
}
...
function main() {
  ...
  const js = `export const shishiClothesData = ${JSON.stringify(data, null, 2)}\n`
