...
function parseArgs() {
  ...
  const out = { xlsx: '', out: path.join(process.cwd(), 'src/data/jinjiangClothes.js'), sheet: '' }
...
function transformRow(r) {
  ...
  const paidInCapital = firstNonEmpty(r, ['实缴资本','实收资本','实缴出资','实缴资本(万元)','实收资本(万元)','注册资本实缴'])
...
function smartParseSheet(sh) {
  ...
  if (matrix.length >= 2) {
    ...
    let hdrIdx = -1
    const maxScan = Math.min(300, matrix.length)
    for (let i = 0; i < maxScan; i++) {
      const row = matrix[i].map(v => sanitize(v))
      const matched = patterns.reduce((acc, rgx) => acc + (row.some(cell => rgx.test(cell)) ? 1 : 0), 0)
      if (matched >= 4) { hdrIdx = i; break }
    }
    if (hdrIdx === -1) hdrIdx = 1
    const hdr = matrix[hdrIdx].map(v => String(v || '').trim())
    const dataRows = matrix.slice(hdrIdx + 1).map(arr => Object.fromEntries(arr.map((v, i) => [hdr[i] || `COL_${i}`, v])))
    headerKeys = hdr
    if (!isBadHeaders(headerKeys)) return { rows: dataRows, headerKeys, rangeUsed: hdrIdx }
  }
...
function main() {
  ...
  if (!chosenRows.length) {
    ...
    let validRows = []
    let validHeaderKeys = []
    let validRangeUsed = 0
    for (const sn of wb.SheetNames) {
      const sh = wb.Sheets[sn]
      const parsed = smartParseSheet(sh)
      const valid = !isBadHeaders(parsed.headerKeys) && parsed.rows.length > 0
      console.log(`Sheet '${sn}' -> rows: ${parsed.rows.length} (range shift: ${parsed.rangeUsed}) [valid=${valid}]`)
      ...
      if (valid && parsed.rows.length > (validRows.length || 0)) { validRows = parsed.rows; validSheetName = sn; validHeaderKeys = parsed.headerKeys; validRangeUsed = parsed.rangeUsed }
    }
    if (validRows.length) {
      chosenRows = validRows
      chosenSheetName = validSheetName
      console.log(`Using VALID sheet '${chosenSheetName}' with ${chosenRows.length} rows (range shift: ${validRangeUsed})`)
      console.log('Detected headers:', (validHeaderKeys || []).join(' | '))
    } else {
      chosenRows = bestRows
  ...
  const js = `export const jinjiangClothesData = ${JSON.stringify(data, null, 2)}\n`
