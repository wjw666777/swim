#!/usr/bin/env node
/**
 * Convert an XLSX file to a JS module exporting `lalianData`.
 * Usage:
 *   node scripts/build_lalian_data.js --xlsx ./path/to/file.xlsx [--out ./src/data/lalian.js] [--sheet <nameOrIndex>]
 */
import fs from 'fs'
...
function parseArgs() {
  ...
  const out = { xlsx: '', out: path.join(process.cwd(), 'src/data/lalian.js'), sheet: '' }
...
function transformRow(r, idx) {
  ...
  // Phones: ensure mapping supports “有效手机号”和“更多电话号码”
  const usefulPhone = firstNonEmpty(r, ['有效手机号','有效电话号码','有用电话','对外电话','电话','联系电话'])
    || firstNonEmptyByRegex(r, [/有效.*(手机号|手机|电话)|联系电话|电话/i])
  const morePhone = firstNonEmpty(r, ['更多电话号码','更多电话','其它电话','备用电话','手机'])
...
function main() {
  ...
  const js = `export const lalianData = ${JSON.stringify(data, null, 2)}\n`
