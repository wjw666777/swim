...
function parseArgs() {
  ...
  const out = { xlsx: '', out: path.join(process.cwd(), 'src/data/yinglinClothes.js'), sheet: '' }
...
function parseRegion(addr) {
  if (!addr) return { province: '', city: '', district: '' }
  const t = String(addr)
  let p = '', c = '', d = ''
  const pi = t.indexOf('省') >= 0 ? t.indexOf('省') : t.indexOf('自治区')
  const ci = t.indexOf('市')
  const di = t.indexOf('区') >= 0 ? t.indexOf('区') : t.indexOf('县')
  try {
    if (pi > 0) p = t.slice(0, pi + 1)
    if (ci > 0) {
      const st = p ? pi + 1 : 0
      c = t.slice(st, ci + 1).replace(/^.*[省自治区]/, '')
    }
    if (di > 0) {
      const st = c ? ci + 1 : (p ? pi + 1 : 0)
      d = t.slice(st, di + 1).replace(/^.*市/, '')
    }
  } catch {}
  return { province: p.trim(), city: c.trim(), district: d.trim() }
}

function transform(r) {
  const company = val(r, ['企业名称','公司名称','公司名','名称']) || valR(r, [/公司.*名|企业.*名|公司.*称|企业.*称/i])
  const boss = val(r, ['法定代表人','老板','负责人','联系人','法人']) || valR(r, [/法定代表|法人|负责人|联系人|老板/i])
  ...
  const address = val(r, ['注册地址','地址','企业地址']) || valR(r, [/地址|注册地址/i])
  ...
  if ((!province || !city) && address) {
    const rg = parseRegion(address)
    province = province || rg.province
    city = city || rg.city
    district = district || rg.district
  }
  ...
  const profile = val(r, ['企业简介','公司简介','简介']) || valR(r, [/简介/i])
  const businessScope = val(r, ['经营范围']) || valR(r, [/经营范围|业务范围/i])
...
function bad(keys) {
  ...
  const all = keys.every(k => !String(k).trim() || /^_\d+$/.test(String(k)) || String(k).includes('__EMPTY'))
  const has = keys.some(k => /公司|企业|名称|地址|电话|法人|负责人|省|市|区|注册|参保|规模|简介|经营范围|实缴|实收/.test(String(k)))
  return all || !has
}
...
function parseSheet(sh) {
  ...
  for (let r = 1; r <= 3; r++) {
    ...
    hdr = rows.length ? Object.keys(rows[0]) : []
    if (!bad(hdr)) return { rows, headerKeys: hdr, rangeUsed: r }
  }
  const matrix = XLSX.utils.sheet_to_json(sh, { defval: '', header: 1 })
  if (matrix.length >= 2) {
    const h = matrix[1].map(v => String(v || '').trim())
    const data = matrix.slice(2).map(arr => Object.fromEntries(arr.map((v, i) => [h[i] || `COL_${i}`, v])))
    hdr = h
    if (!bad(hdr)) return { rows: data, headerKeys: hdr, rangeUsed: 1 }
  }
...
function main() {
  ...
  if (sheet) {
    ...
    if (wb.SheetNames.includes(tn)) {
      const sh = wb.Sheets[tn]
      const p = parseSheet(sh)
      chosen = p.rows
      sheetName = tn
    }
  }
  if (!chosen.length) {
    ...
    for (const sn of wb.SheetNames) {
      ...
      const p = parseSheet(sh)
      if (p.rows.length > bestRows.length) { bestRows = p.rows; best = sn }
    }
    chosen = bestRows
    sheetName = best
  }
  ...
  fs.writeFileSync(out, `export const yinglinClothesData = ${JSON.stringify(data, null, 2)}\n`, 'utf8')
}
