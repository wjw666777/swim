...
function parseArgs() {
  ...
  const out = { xlsx: '', out: path.join(process.cwd(), 'src/data/quanzhouMens.js'), sheet: '' }
...
function parseRegion(addr) {
  ...
  try {
    ...
    if (ci > 0) { const st = p ? pi + 1 : 0; c = t.slice(st, ci + 1).replace(/^.*[省自治区]/, '') }
    if (di > 0) { const st = c ? ci + 1 : (p ? pi + 1 : 0); d = t.slice(st, di + 1).replace(/^.*市/, '') }
  } catch {}
...
function bad(keys) { if (!keys || !keys.length) return true; const all = keys.every(k => !String(k).trim() || /^_\d+$/.test(String(k)) || String(k).includes('__EMPTY')); const has = keys.some(k => /公司|企业|名称|地址|电话|法人|负责人|省|市|区|注册|参保|规模|简介|经营范围|实缴|实收/.test(String(k))); return all || !has }
...
function main() {
  ...
  fs.writeFileSync(out, `export const quanzhouMensData = ${JSON.stringify(data, null, 2)}\n`, 'utf8')
}
