...
function parseArgs() {
  ...
  const out = { xlsx: '', out: path.join(process.cwd(), 'src/data/dongguanPrinting.js'), sheet: '' }
...
function transform(r) {
  const company = val(r, ['企业名称','公司名称','工厂名称','名称']) || valR(r, [/企业.*名|公司.*名|工厂.*名|名称/i])
  ...
  const address = val(r, ['注册地址','地址','企业地址','工厂地址']) || valR(r, [/地址|注册地址/i])
  ...
  const profile = val(r, ['企业简介','公司简介','工厂简介','简介']) || valR(r, [/简介/i])
  const businessScope = val(r, ['经营范围','主营','业务范围']) || valR(r, [/经营范围|业务范围|主营/i])
...
function bad(keys) { if (!keys || !keys.length) return true; const all = keys.every(k => !String(k).trim() || /^_\d+$/.test(String(k)) || String(k).includes('__EMPTY')); const has = keys.some(k => /公司|企业|工厂|名称|地址|电话|法人|负责人|省|市|区|注册|参保|规模|简介|经营范围|主营|实缴|实收/.test(String(k))); return all || !has }
...
function main() {
  ...
  fs.writeFileSync(out, `export const dongguanPrintingData = ${JSON.stringify(data, null, 2)}\n`, 'utf8')
}
