#!/usr/bin/env node
/**
 * Convert an XLSX file to a JS module exporting `fabricData`.
 * Usage:
 *   node scripts/build_fabric_data.js --xlsx ./path/to/file.xlsx [--out ./src/data/fabric.js] [--sheet <nameOrIndex>]
 */
import fs from 'fs'
...
function parseArgs() {
  ...
  const out = { xlsx: '', out: path.join(process.cwd(), 'src/data/fabric.js'), sheet: '' }
...
// Fallback: match by regex against all headers when exact keys fail
function firstNonEmptyByRegex(obj, regexes) {
  const keys = Object.keys(obj)
  for (const rgx of regexes) {
    const k = keys.find((kk) => rgx.test(String(kk)))
    if (k && obj[k] !== undefined && obj[k] !== null && String(obj[k]).trim() !== '') {
      return String(obj[k]).trim()
    }
  }
  return ''
...
function transformRow(r, idx) {
  ...
  const registerStatus = firstNonEmpty(r, ['登记状态','状态'])
    || firstNonEmptyByRegex(r, [/登记|状态/i])
  ...
  const createTime = firstNonEmpty(r, ['成立日期','成立时间','注册日期'])
    || firstNonEmptyByRegex(r, [/成立.*(日|期|时间)|注册.*(日|期)/i])
  ...
  // Map phones: 用户指定“有效手机号” -> usefulPhone，“更多电话” -> morePhone
  const usefulPhone = firstNonEmpty(r, ['有效手机号','有用电话','对外电话','电话','联系电话'])
    || firstNonEmptyByRegex(r, [/有效.*(手机号|手机)|电话|联系电话/i])
  const morePhone = firstNonEmpty(r, ['更多电话','其它电话','备用电话','手机'])
    || firstNonEmptyByRegex(r, [/手机|备用|其它|其他.*电话/i])
  const emailRaw = firstNonEmpty(r, ['邮箱','电子邮箱','Email','email'])
    || firstNonEmptyByRegex(r, [/邮箱|email/i])
  const email = (emailRaw || '').replace(/；/g,';').replace(/\s+/g,'')
  const companyType = firstNonEmpty(r, ['公司类型','企业类型','类型'])
    || firstNonEmptyByRegex(r, [/类型/i])
  const people = firstNonEmpty(r, ['人员规模','人数','从业人数'])
    || firstNonEmptyByRegex(r, [/人数|人员|从业.*人数/i])
...
function isBadHeaders(keys) {
  ...
  const hasMeaningful = keys.some(k => /公司|企业|名称|地址|电话|邮箱|法人|负责人|省|市|区|成立|注册/.test(String(k)))
...
function smartParseSheet(sh) {
  ...
  // Try shifting range to skip title rows (up to 3 rows)
  for (let r = 1; r <= 3; r++) {
  ...
  // Fallback: header:1 (array-of-arrays)
  const matrix = XLSX.utils.sheet_to_json(sh, { defval: '', header: 1 })
  // Use second row as header if possible
  if (matrix.length >= 2) {
  ...
  // Final fallback: build objects by COL_i keys
  ...
  return { rows: dataRows, headerKeys: hdr, rangeUsed: 0 }
}
...
function main() {
  ...
  const js = `export const fabricData = ${JSON.stringify(data, null, 2)}\n`
